...
app.post('/candidate', upload1.single('resume'), (req, res) => {
  ...
  // Ensure that the resume path is properly retrieved
  const resumePath = req.file ? req.file.path : null;

  // MySQL insert query
  const sql = `
      INSERT INTO candidate (candidate_name, candidate_email, mobile_no, resume, experience, job_portal, refered_by, current_location, preferred_location, expected_salary, notice_period)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  const values = [
    candidate_name,
  ...
  try {
      ...
      const sql = `
          ...
          SET candidate_name = ?, candidate_email = ?, mobile_no = ?, experience = ?, job_portal = ?, refered_by = ?,
              current_location = ?, preferred_location = ?, expected_salary = ?, notice_period = ?
          WHERE candidate_id = ?
      `;
      const values = [
        candidate_name,
...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
      user: "noreply.samarthainfosolutions@gmail.com",
      pass: "upyoyvgxslulphgt",
  },
});
...
app.post('/requests', (req, res) => {
  upload(req, res, (err) => {
    ...
    db.query(sql, values, (err, result) => {
      ...
      const emailAddresses = recruiter_email.split(',').map(email => email.trim());
      const mailContent = {
        from: 'noreply.samarthainfosolutions@gmail.com',
        to: emailAddresses, // Split the recruiter_email field by commas to handle multiple recipients
...
app.get('/candidates_for_request/:request_id', (req, res) => {
  const requestId = req.params.request_id;

  const sqlFetchCandidates = `
      SELECT 
          c.candidate_id,
          c.candidate_name,
          c.candidate_email,
      ...
      JOIN 
          candidate c ON cr.candidate_id = c.candidate_id
      WHERE 
          cr.request_id = ?
  `;

  db.query(sqlFetchCandidates, [requestId], (error, results) => {
      if (error) {
          console.error('Error fetching candidates:', error);
          return res.status(500).json({ error: 'Failed to fetch candidate details.' });
      }

      // Convert results to the desired array format
      ...
      res.json(candidateDetailsArray);
  });
});
...
app.put('/requested_candidates/:id', (req, res) => {
  ...
  const {
      ...
      stage_of_interview,
      interviewer_name,
      scheduled_interview_timing,
      type_of_interview,
      additional_details
  } = req.body;

  // Check if mandatory fields are provided
  if (!mode_of_interview || !stage_of_interview || !scheduled_interview_timing || !type_of_interview || !interviewer_name) {
  ...
  // Update requested candidate in the database
  const updateSql = `
      UPDATE requested_candidate
      SET mode_of_interview = ?, stage_of_interview = ?, scheduled_interview_timing = ?, type_of_interview = ?
      WHERE requested_candidate_id = ?
  `;
  const updateValues = [mode_of_interview, stage_of_interview, scheduled_interview_timing, type_of_interview, requestedCandidateId];

  db.query(updateSql, updateValues, (error) => {
      if (error) {
      ...
      db.query(fetchCandidateDetailsQuery, [requestedCandidateId], (err, candidateResult) => {
          ...
          db.query(fetchInterviewerDetailsQuery, [interviewer_name], (err, interviewerResult) => {
              ...
              const interviewerEmail = interviewerResult[0].interviewer_email;

              // Only send email if type_of_interview is not Telephonic
              if (type_of_interview !== 'Telephonic') {
                  sendEmail(candidate_email, interviewerEmail, scheduled_interview_timing, mode_of_interview, type_of_interview, additional_details)
                  .then(() => {
                  ...
                  res.json({ message: 'Requested candidate updated successfully, no email notifications sent for Telephonic interviews.' });
              }
          });
      });
  });
...
function sendEmail(candidateEmail, interviewerEmail, scheduledInterviewTiming, modeOfInterview, typeOfInterview, additionalDetails = null) {
  return new Promise((resolve, reject) => {
      ...
      const fetchCandidateNameQuery = `
          SELECT candidate_name
          FROM candidate
          WHERE candidate_email = ?
      `;
      db.query(fetchCandidateNameQuery, [candidateEmail], (err, candidateResult) => {
          if (err) {
          ...
          db.query(fetchSenderEmailQuery, (err, senderEmailResult) => {
              ...
              const fetchInterviewerNameQuery = `
                  SELECT ename AS interviewer_name
                  FROM ud
                  WHERE username = ?
              `;
              db.query(fetchInterviewerNameQuery, [interviewerEmail], (err, interviewerResult) => {
                  if (err) {
                      console.error('Error fetching interviewer name:', err);
                      reject(err);
                      return;
                  }
...
Attached is the candidate's resume for your reference.
...
router.put('/candidates/:candidate_id/status', (req, res) => {
  const candidateId = req.params.candidate_id;
  const { status, requestId } = req.body;

  // Validate input
  if (!status || !requestId) {
      return res.status(400).json({ error: 'Status and requestId fields are required.' });
  }

  // Implement logic to update candidate status in your database
  const sql = 'UPDATE candidate SET status = ? WHERE candidate_id = ?';
  db.query(sql, [status, candidateId], (error, results) => {
      if (error) {
      ...
      // Optionally, you can update the status in the associated request or perform other actions based on your application logic.

      res.status(200).json({ message: `Candidate ${candidateId} status updated successfully.` });
  });
});


app.get('/selected_candidates_for_request/:request_id', (req, res) => {
  const requestId = req.params.request_id;
  ...
  const sqlFetchCandidates = `
    ...
    JOIN 
      candidate c ON cr.candidate_id = c.candidate_id
    WHERE 
      cr.request_id = ? AND c.status = 'selected'
  `;
...
app.post('/add_candidates_to_request', (req, res) => {
  const { candidate_ids, request_id } = req.body;

  if (!Array.isArray(candidate_ids) || candidate_ids.length === 0) {
      return res.status(400).json({ error: 'No candidates provided.' });
  }

  const sqlInsertAssociations = `
      INSERT INTO candidate_requests (candidate_id, request_id)
      VALUES ?
  `;
  
  const values = candidate_ids.map(candidateId => [candidateId, request_id]);

  db.query(sqlInsertAssociations, [values], (error) => {
      if (error) {
          console.error('Error inserting candidate-request associations:', error);
          return res.status(500).json({ error: 'Failed to associate candidates with request.' });
      }

      const sqlFetchDetails = `
          SELECT 
              ud.ename AS interviewer_name, 
              ud.username AS interviewer_email,
              COUNT(cr.candidate_id) AS candidateCount
          FROM 
              request r
          JOIN 
              ud ON r.eid = ud.eid
          LEFT JOIN 
              candidate_requests cr ON r.request_id = cr.request_id
          WHERE 
              r.request_id = ?
          GROUP BY 
              ud.ename, ud.username;
      `;
  
      db.query(sqlFetchDetails, [request_id], (fetchError, results) => {
          if (fetchError) {
              console.error('Error fetching interviewer details:', fetchError);
              return res.status(500).json({ error: 'Failed to fetch interviewer details.' });
          }
          ...
          const { interviewer_name, interviewer_email, candidateCount } = results[0];
  
          // Send email notification to interviewer
          const mailOptions = {
              from: 'noreply.samarthainfosolutions@gmail.com',
              to: interviewer_email,
              subject: 'Candidates Added to Your Job Request',
              text: `Dear ${interviewer_name},

             We have added ${candidateCount} candidates to your job request. Please log in to review the candidates and select the ones you wish to proceed with for further evaluation.
      ...
      Samartha InfoSolutions Pvt Ltd.`
          ...
          transporter.sendMail(mailOptions, (emailError, info) => {
              ...
              res.status(200).json({ message: 'Candidates processed successfully.', candidateCount });
          });
      });
  });
});
